...
#define THISFIRMWARE "ArduPlaneNG V2x R6 for AIOP v1.0 by JLN" // New setup for CRIUS_AIO_PRO_V1
/*
...
//
// Jean-Louis Naudin (JLN) updates: last update 2013-12-06
//
// DOLIST:
//
//-------------------------------------------------------------------------------------------------------------------------
// 2013-12-06: R6 successfully tested in flight on the Maxi Swift flying wing in AUTO mode under FPL and AUTO_SAVE_WP
// 2013-12-04: Update for AGL navigation under FPL
// 2012-08-15: Tested in flight with the Maxi Swift flying wing and the Crius GPS CN-06 Ublox NEO-6(woow) (see: https://vimeo.com/47595388 )
// 2012-08-02: Tested in flight with the EasyGlider and GPS MT3329 (woow) (see: https://vimeo.com/46825370 )
// 2012-08-01: Tests: compass, airpressure, airspeed OK for the AIOP v1
// 2012-08-01: Updated for PIRATES_SENSOR_BOARD PIRATES_CRIUS_AIO_PRO_V1
// 2012-07-15: Added Xplane 9.70/AeroSim 3.91 compatibility for HIL simulations
// 2012-07-11: Tested in flight on Easyglider with ArduplaneNG v2.4r2
// 2012-07-11: In stabilize mode no throttle limitation as in manual
// 2012-07-11: Add closed loop nav, Auto_WP_Radius, SAVE_WP and Reset FPL via SW on CH7
// 2012-07-11: Update throttle value for AIOP
// 2012-07-30: Porting Arduplane v2.50 official to ArduplaneNG v2.5 for AIOP v1.0 board (All In One Pro)
//
//-------------------------------------------------------------------------------------------------------------------------
*/
////////////////////////////////////////////////////////////////////////////////
...
static AP_Int8		*flight_modes = &g.flight_mode1;
	...
	// real sensors
	    ...
	    AP_Compass_HIL          compass;
			...
			#endif
		#elif CONFIG_BARO == AP_BARO_MS5611_I2C
		    AP_Baro_MS5611_I2C barometer;
		#elif CONFIG_BARO == AP_BARO_MS5611
		    AP_Baro_MS5611 barometer;
		#elif CONFIG_BARO == AP_BARO_BMP085_PIRATES
...
// JLN Update - ThermoPilot
static int    back2home = 0;
static boolean        headalt_set		= false;
unsigned long  timesw                  = 0;
static long    alt2lock                = 0;
static long    head2lock               = 0;
static long    runway                  = 0;
static long    dtGPS                   = 0;
static long    GPStimer                = 0;
static int16_t Vz                      = 0;
static boolean landing_point_set       = false;
static boolean roll_set                = false; 
unsigned long timesw2           = 0;
static int  rudder                     = 0;
static int  roll                     = 0;
static int  turn                       = 0;
static int roll_dir                    = 0;
static int roll_step                   = 0;
static long angle_of_roll              = 0;
static long old_angle_of_roll          = 0;
static int roltoff                     = 900;
...
#if CONFIG_APM_HARDWARE == APM_HARDWARE_PIRATES
                                g.channel_throttle.servo_out = g.throttle_max * 10;
#else
...
/*
  ...
  */                              
				...
				if (land_complete){
                                         nav_roll = 0; // hold the pitch up smoothly for a kiss landing.
                                        nav_pitch = landing_pitch + ((float)(current_loc.alt - next_WP.alt) * 3.5);
                                        if (climb_rate > 0) {    // never goes up again...
                                           nav_pitch = 0;
                                        }                                                                             
					g.channel_throttle.servo_out = 0;
                                        if(g_gps->ground_speed == 0) {
                                          nav_pitch = 0;

                                        }
                                      else if(airspeed < 1100) {
                                        g.spe_aoa = 50;    // hold the pitch up to slow down
                                        } 
				}
				break;
...
#if CONFIG_APM_HARDWARE == APM_HARDWARE_PIRATES
  					g.channel_throttle.servo_out = g.throttle_cruise * 10;
#else
...
static void update_alt()
{
    ...
    if (barometer.healthy) {
        ...
        current_loc.alt += g.altitude_mix * (read_barometer() + home.alt);
          ...
          current_loc.alt = max(baro_alt + home.alt, 0);   // alt_AGL          
        }    
