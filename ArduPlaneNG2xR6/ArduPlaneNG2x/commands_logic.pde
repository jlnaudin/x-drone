...
static bool verify_takeoff()
{
	...
	if (current_loc.alt > takeoff_altitude)  {
		hold_course = -1;
	        target_bearing 	= get_bearing(&current_loc, &next_WP);    // JLN update
...
{
	    // JLN update
	if (((wp_distance > 0) && (wp_distance <= (10*g_gps->ground_speed/100)))) {
           if (current_loc.alt <= next_WP.alt + 800){            
  	       land_complete = 1;		//Set land_complete within 8 m AGL                     
           } else if (current_loc.alt <= next_WP.alt + 1000) {   // final for landing, if we are within 10 seconds distance or 10 m AGL
               final = true;
               landing_pitch = 800;           
            }                
		if(hold_course == -1){
