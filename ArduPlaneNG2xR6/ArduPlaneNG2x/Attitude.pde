...
static void stabilize()
{
	...
	long tempcalc = nav_pitch + (g.spe_aoa * 100) +
...
static void calc_throttle()
{
  if (g.airspeed_enabled == false || g.airspeed_use == false) {
	...
	int throttle_target = (g.throttle_cruise + throttle_nudge) * 10;
    #else
    ...
    // control?
...
#if CONFIG_APM_HARDWARE == APM_HARDWARE_PIRATES
		if (nav_pitch >= 0) {
			g.channel_throttle.servo_out = throttle_target + ((g.throttle_max*10) - throttle_target) * nav_pitch / g.pitch_limit_max;
		} else {
			g.channel_throttle.servo_out = throttle_target - (throttle_target - (g.throttle_min*10)) * nav_pitch / g.pitch_limit_min;
		}

		g.channel_throttle.servo_out = constrain(g.channel_throttle.servo_out, g.throttle_min.get()*10, g.throttle_max.get()*10);
	} else {
		...
		g.channel_throttle.servo_out = (g.throttle_cruise*10) + (g.pidTeThrottle.get_pid(energy_error, dTnav)*10);
		g.channel_throttle.servo_out += (g.channel_pitch.servo_out * g.kff_pitch_to_throttle);

                int gs_boost = g.gsboost * (1.0 - ((float)g_gps->ground_speed/100) / (float)airspeed/100);
	        gs_boost = max(0,gs_boost);
	        g.channel_throttle.servo_out += (gs_boost * 10);


		g.channel_throttle.servo_out = constrain(g.channel_throttle.servo_out,
			g.throttle_min.get()*10, g.throttle_max.get()*10);			// TODO - resolve why "saved" is used here versus "current"
#else
		if (nav_pitch >= 0) {
		...
		g.channel_throttle.servo_out += (g.channel_pitch.servo_out * g.kff_pitch_to_throttle);
	        ...
	        // lower the GSBOOST constant value to limit the effect : 100 = default

            if(g.auto_thermal == DISABLED)
            {   int gs_boost = g.gsboost * (1.0 - ((float)g_gps->ground_speed/100) / (float)airspeed/100);
	        gs_boost = max(0,gs_boost);
	        g.channel_throttle.servo_out += gs_boost;
             }
...
static void set_servos(void)
{
	...
	if(control_mode == MANUAL){
		...
		#else
                if(control_mode != STABILIZE){          // if in STABILIZE mode not throttle limitation
