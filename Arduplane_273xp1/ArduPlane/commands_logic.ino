...
static bool verify_nav_wp()
{
    ...
    nav_controller->update_waypoint(prev_WP, next_WP);
        ...
        if(g.auto_wp_radius) 
        { calc_turn_radius();  // JLN ThermoPilot - auto-adap the wp_radius Vs the gspeed and max roll angle
	if ((wp_distance > 0) && (wp_distance <= wp_radius)) {
        ...
        } else {    if (wp_distance <= nav_controller->turn_distance(g.waypoint_radius)) {
        gcs_send_text_fmt(PSTR("Reached Waypoint #%i dist %um"),
                          (unsigned)nav_command_index,
                          (unsigned)get_distance(&current_loc, &next_WP));
        return true;
...
static bool verify_REFpoint()    // JLN Thermopilot
{
        update_loiter();
	if (wp_distance <= (uint32_t)max(g.waypoint_radius,0) || 
        nav_controller->reached_loiter_target()) {
	gcs_send_text_P(SEVERITY_LOW,PSTR("Reached REFpoint"));
	return true;
    } else {
        return false;
	}
