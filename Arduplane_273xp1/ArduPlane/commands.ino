...
/*        
	...
	struct Location temp = get_cmd_with_index(0);    // JLN update - for HIL test only get the home param stored in the FPL
        if (temp.alt > 0) {
         ...
         if (g.msl_ref) {   // MSL ref
                gps_base_alt    = (int32_t)g.agl_ref * 100;             // get the stored home altitude as the base ref for AGL calculation.
                home.alt        = gps_base_alt;   
         } else {            // AGL ref from the local airport
                gps_base_alt    = max(g_gps->altitude, 0);             // get the stored home altitude as the base ref for AGL calculation.
                home.alt        = max(g_gps->altitude, 0);                
               // home.alt        = 0;
                 }
#endif
